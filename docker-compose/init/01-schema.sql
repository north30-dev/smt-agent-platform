-- 此文件为 Docker 环境唯一 DDL 入口，合并自 docs/database/init.sql (Phase 1-2) 与 shared/db_schema.sql (Phase 3)。
-- Phase 3 修复批次补全 Phase 1-2 表与 workflows 表。
-- 全新环境 docker compose up 后，PostgreSQL 镜像通过 /docker-entrypoint-initdb.d 自动按字母序执行
-- init/01-schema.sql → 02-seed-devices.sql，确保所有表与种子数据就位。
